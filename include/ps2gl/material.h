...
class CMaterial {
   ...
   public:
      CMaterial(CGLContext &context) : GLContext(context) {}
...
class CImmMaterial : public CMaterial {
   ...
   public:
      ...
      void SetEmission( cpu_vec_xyzw emission ) { Emission = emission; TellRendererMaterialChanged(); }
      void SetShininess( float shine ) { Shininess = shine; TellRendererMaterialChanged(); }
...
class CDListMaterial : public CMaterial {
   ...
   public:
      CDListMaterial( CGLContext &context ) : CMaterial(context) {}
...
class CMaterialManager {
   ...
   public:
	 ...
	 : GLContext(context),
	   ImmMaterial(context), DListMaterial(context),
	 ...
	 ImmMaterial.SetDiffuse( cpu_vec_xyzw(0.8f, 0.8f, 0.8f, 1.0f) );
      ...
      void BeginDListDef() { CurMaterial = &DListMaterial; InDListDef = true; }
      void EndDListDef() { CurMaterial = &ImmMaterial; InDListDef = false; }
};
