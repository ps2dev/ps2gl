...
void
CImmDrawContext::SetDrawBuffers( bool interlaced,
				 ...
				 GS::CMemArea *depthMem )
{
   ...
   if ( depthMem ) depthPsm = depthMem->GetPixFormat();
   DepthBits = 0;
   if ( depthPsm == GS::kPsmz24 ) DepthBits = 24;
   else if ( depthPsm == GS::kPsmz32 ) DepthBits = 28; // for fog
   else if ( depthPsm == GS::kPsmz16 ) DepthBits = 16;
   else { mError( "Unknown depth buffer format" ); }
...
void
CImmDrawContext::SwapBuffers( bool fieldIsEven )
{
...
void
CImmDrawContext::SetDoSmoothShading( bool yesNo )
{
...
void
CImmDrawContext::SetDoClipping( bool clip )
{
   if ( DoClipping != clip ) {
      DoClipping = clip;
...
void
CImmDrawContext::SetBlendEnabled( bool enabled )
{
...
void
CImmDrawContext::SetDepthWriteEnabled( bool enabled )
{
...
void
CImmDrawContext::SetPolygonMode( GLenum mode )
{
   if ( PolyMode != mode ) {
      PolyMode = mode;
...
void
CImmDrawContext::SetInterlacingOffset( float yPixels )
{
...
void
CImmDrawContext::SetBlendMode( GLenum source, GLenum dest )
{
...
class CSetDrawBuffers : public CDListCmd {
   ...
   public:
	 ...
	 : Frame0(frame0), Frame1(frame1), Depth(depth), Interlaced(inter) {}
      CDListCmd* Play() {
...
void
CDListDrawContext::SetDrawBuffers( bool interlaced,
				   GS::CMemArea *frame0Mem, GS::CMemArea *frame1Mem,
				   GS::CMemArea *depthMem )
{
...
class CSetDoSmoothShadingCmd : public CDListCmd {
      bool	DoSS;
   public:
      CSetDoSmoothShadingCmd( bool yesNo ) : DoSS(yesNo) {}
      CDListCmd* Play() {
...
void
CDListDrawContext::SetDoSmoothShading( bool yesNo )
{
   GLContext.GetDListGeomManager().Flush();
...
class CSetDoClippingCmd : public CDListCmd {
      bool	DoClip;
   public:
      CSetDoClippingCmd(bool clip) : DoClip(clip) {}
      CDListCmd* Play() {
...
void
CDListDrawContext::SetDoClipping( bool clip )
{
   GLContext.GetDListGeomManager().Flush();
...
class CSetDoCullFaceCmd : public CDListCmd {
      bool	DoCull;
   public:
      CSetDoCullFaceCmd(bool cull) : DoCull(cull) {}
      CDListCmd* Play() {
...
void
CDListDrawContext::SetDoCullFace( bool cull )
{
   GLContext.GetDListGeomManager().Flush();
...
class CSetCullFaceDir : public CDListCmd {
      int	CullDir;
   public:
      CSetCullFaceDir(int dir) : CullDir(dir) {}
      CDListCmd* Play() {
...
void
CDListDrawContext::SetCullFaceDir( int direction )
{
   GLContext.GetDListGeomManager().Flush();
...
class CSetBlendEnabledCmd : public CDListCmd {
      bool	Enabled;
   public:
      CSetBlendEnabledCmd( bool enabled ) : Enabled(enabled) {}
      CDListCmd* Play() {
...
void
CDListDrawContext::SetBlendEnabled( bool enabled )
{
   GLContext.GetDListGeomManager().Flush();
...
class CSetAlphaTestEnabledCmd : public CDListCmd {
      bool	Enabled;
   public:
      CSetAlphaTestEnabledCmd( bool enabled ) : Enabled(enabled) {}
      CDListCmd* Play() {
...
void
CDListDrawContext::SetAlphaTestEnabled( bool enabled )
{
   GLContext.GetDListGeomManager().Flush();
...
class CSetInterlacingOffsetCmd : public CDListCmd {
      float	Offset;
   public:
      CSetInterlacingOffsetCmd( float offset ) : Offset(offset) {}
      CDListCmd* Play() {
...
void
CDListDrawContext::SetInterlacingOffset( float yPixels )
{
   GLContext.GetDListGeomManager().Flush();
...
class CSetAlphaFuncCmd : public CDListCmd {
    ...
    GLclampf Ref;
public:
    CSetAlphaFuncCmd( GLenum func, GLclampf ref ) : Func(func), Ref(ref) {}
    CDListCmd* Play() {
...
class CSetRescaleNormalsCmd : public CDListCmd {
      bool	Rescale;
   public:
      CSetRescaleNormalsCmd( bool rescale ) : Rescale(rescale) {}
      CDListCmd* Play() {
...
void
CDListDrawContext::SetRescaleNormals( bool rescale )
{
   GLContext.GetDListGeomManager().Flush();
...
class CSetBlendMode : public CDListCmd {
      GLenum	Source, Dest;
   public:
      CSetBlendMode( GLenum s, GLenum d ) : Source(s), Dest(d) {}
      CDListCmd* Play() {
...
void
CDListDrawContext::SetBlendMode( GLenum source, GLenum dest )
{
   GLContext.GetDListGeomManager().Flush();
...
class CSetDepthWriteEnabledCmd : public CDListCmd {
      bool	Enabled;
   public:
      CSetDepthWriteEnabledCmd( bool enabled ) : Enabled(enabled) {}
      CDListCmd* Play() {
...
void
CDListDrawContext::SetDepthWriteEnabled( bool enabled )
{
   GLContext.GetDListGeomManager().Flush();
...
class CSetFrameBufferDrawMaskCmd : public CDListCmd {
      unsigned int	Mask;
   public:
      CSetFrameBufferDrawMaskCmd( unsigned int mask ) : Mask(mask) {}
      CDListCmd* Play() {
...
void
CDListDrawContext::SetFrameBufferDrawMask( unsigned int mask )
{
   GLContext.GetDListGeomManager().Flush();
		  ...
		  GLboolean b_enabled, GLboolean a_enabled)
{
   CImmDrawContext &drawContext = pGLContext->GetImmDrawContext();
   unsigned int mask=0;
   if (r_enabled==GL_FALSE) mask|=0xff;
   if (g_enabled==GL_FALSE) mask|=0xff00;
   if (b_enabled==GL_FALSE) mask|=0xff0000;
   if (a_enabled==GL_FALSE) mask|=0xff000000;
 ...
 */
void
pglSetDrawBuffers( int interlaced,
...
void
pglSetInterlacingOffset( float yPixels )
{
