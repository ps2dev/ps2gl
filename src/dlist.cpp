...
CDList::CDList()
   : VertexBuf(NULL), NormalBuf(NULL), TexCoordBuf(NULL), ColorBuf(NULL),
     NumRenderPackets(0)
{
#ifndef PS2_LINUX
...
bool
CDListManager::ListsAreFree( int firstListID, int numLists )
{
...
void
CDListManager::DeleteLists( unsigned int firstListID, int numLists )
{
   for ( int i = 0; i < numLists; i++ ) {
...
void
CDListManager::NewList( unsigned int listID, GLenum mode )
{
...
void
CDListManager::EndList()
{
   mErrorIf( OpenListID == 0, "There is no list to end!" );

   if ( Lists[OpenListID] ) AddListToBeFreed( Lists[OpenListID] );
...
class CCallListCmd : public CDListCmd {
      unsigned int	ListID;
   public:
      CCallListCmd( unsigned listID ) : ListID(listID) {}
      CDListCmd* Play() {
...
void
CDListManager::CallList( unsigned int listID )
{
   if ( OpenListID != 0 ) {
