...
CImmLight::CImmLight( CGLContext &context, int lightNum )
   : CLight(context, lightNum),
     ...
     SpotCutoff(180.0f), SpotExponent(0.0f),
     ConstantAtten(1.0f), LinearAtten(0.0f), QuadAtten(0.0f),
     bIsEnabled(false),
     Type(kDirectional)
{
}
...
void
CImmLight::CheckTypeChange( tLightType oldType )
{
...
void
CImmLight::SetEnabled( bool enabled )
{
   if ( bIsEnabled != enabled ) {
      bIsEnabled = enabled;
      if (enabled) NumLights[Type]++;
      else NumLights[Type]--;
...
void
CImmLight::SetSpecular( cpu_vec_xyzw specular )
{
...
class CLightPropCmd : public CDListCmd {
   ...
   public:
	 ...
	 : LightNum(lightNum), Property(prop), Value(value) {}
      CDListCmd* Play() {
...
void
CDListLight::SetAmbient( cpu_vec_xyzw ambient )
{
...
void
CDListLight::SetDiffuse( cpu_vec_xyzw diffuse )
{
   GLContext.GetDListManager().GetOpenDList()
...
void
CDListLight::SetSpecular( cpu_vec_xyzw specular )
{
   GLContext.GetDListManager().GetOpenDList()
...
void
CDListLight::SetPosition( cpu_vec_xyzw position )
{
   GLContext.GetDListManager().GetOpenDList()
...
void
CDListLight::SetDirection( cpu_vec_xyzw direction )
{
   GLContext.GetDListManager().GetOpenDList()
...
void
CDListLight::SetSpotDirection( cpu_vec_xyzw dir )
{
   GLContext.GetDListManager().GetOpenDList()
...
void
CDListLight::SetSpotCutoff( float cutoff )
{
   GLContext.GetDListManager().GetOpenDList()
...
void
CDListLight::SetSpotExponent( float exp )
{
   GLContext.GetDListManager().GetOpenDList()
...
void
CDListLight::SetConstantAtten( float atten )
{
   GLContext.GetDListManager().GetOpenDList()
...
void
CDListLight::SetLinearAtten( float atten )
{
   GLContext.GetDListManager().GetOpenDList()
...
void
CDListLight::SetQuadAtten( float atten )
{
   GLContext.GetDListManager().GetOpenDList()
...
void
CDListLight::SetEnabled( bool yesNo )
{
   GLContext.GetDListManager().GetOpenDList()
...
CImmLighting::CImmLighting( CGLContext &context )
   : CLighting(context),
     ...
     Light0(context, 0), Light1(context, 1), Light2(context, 2), Light3(context, 3),
     Light4(context, 4), Light5(context, 5), Light6(context, 6), Light7(context, 7),
     IsEnabled(false),
...
void
CImmLighting::MaterialHasSpecular()
{
   if ( NumLightsWithNonzeroSpecular > 0 ) {
      GLContext.SpecularEnabledChanged();
...
class CSetLightingEnabledCmd : public CDListCmd {
      bool	IsEnabled;
   public:
      CSetLightingEnabledCmd( bool enabled ) : IsEnabled(enabled) {}
      CDListCmd* Play() {
...
void
CDListLighting::SetLightingEnabled( bool enabled )
{
...
class CSetGlobalAmbientCmd : public CDListCmd {
   ...
   public:
      CSetGlobalAmbientCmd( cpu_vec_xyzw newAmb ) : Ambient(newAmb) {}
      CDListCmd* Play() {
...
void
CDListLighting::SetGlobalAmbient( cpu_vec_xyzw newAmb )
{
