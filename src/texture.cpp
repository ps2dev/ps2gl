...
CTexManager::CTexManager( CGLContext &context )
   : GLContext(context), IsTexEnabled(false), InsideDListDef(false),
     Cursor(0), LastTexSent(NULL),
     CurClut(NULL),
...
{
   delete DefaultTex;
   if ( CurClut ) delete CurClut;

   for ( int i = 0; i < NumTexNames; i++ ) {
      if (TexNames[i]) delete TexNames[i];
   }
}
...
class CSetTexEnabledCmd : public CDListCmd {
      bool	IsEnabled;
   public:
      CSetTexEnabledCmd( bool enabled ) : IsEnabled(enabled) {}
      CDListCmd* Play() {
...
void
CTexManager::SetTexEnabled( bool yesNo )
{
   if (! InsideDListDef) {
...
class CSetTexModeCmd : public CDListCmd {
      GS::tTexMode Mode;
   public:
      CSetTexModeCmd( GS::tTexMode mode ) : Mode(mode) {}
      CDListCmd* Play() {
...
void
CTexManager::SetTexMode( GS::tTexMode mode )
{
   if ( ! InsideDListDef )
      TexMode = mode;
   else {
...
void
CTexManager::GenTextures( GLsizei numNewTexNames, GLuint *newTexNames )
{
   for ( int curTexName = 0; curTexName < numNewTexNames; curTexName++ ) {
	 ...
	 if ( Cursor != 0 && TexNames[Cursor] == NULL ) break;
...
void
CTexManager::UseCurTexture( CVifSCDmaPacket &renderPacket )
{
   if ( IsTexEnabled ) {
...
class CBindTextureCmd : public CDListCmd {
   ...
   public:
      CBindTextureCmd( unsigned int name ) : TexName(name) {}
      CDListCmd* Play() {
...
void
CTexManager::BindTexture( GLuint texNameToBind )
{
...
void
CTexManager::DeleteTextures( GLsizei numToDelete, const GLuint *texNames )
{
...
class CSetCurTexParamCmd : public CDListCmd {
      ...
      int	Param;
   public:
      CSetCurTexParamCmd( GLenum pname, int param ) : PName(pname), Param(param) {}
      CDListCmd* Play() {
...
void
CTexManager::SetCurTexParam( GLenum pname, GLint param )
{
   if ( ! InsideDListDef ) {
...
class CSetCurTexImageCmd : public CDListCmd {
   ...
   public:
      CSetCurTexImageCmd( tU128 *image, unsigned int w, unsigned int h,
			  GS::tPSM psm )
	 : Image(image), Width(w), Height(h), Psm(psm) {}
      CDListCmd* Play() {
...
void
CTexManager::SetCurTexImage( tU128* imagePtr, tU32 w, tU32 h,
			     GS::tPSM psm )
{
   GLContext.TextureChanged();
...
class CSetCurClutCmd : public CDListCmd {
   ...
   public:
	 ...
	 : Clut(clut), NumEntries(numEntries) {}
      CDListCmd* Play() {
...
void
CTexManager::SetCurClut( const void *clut, int numEntries )
{
   GLContext.TextureChanged();
...
class CSetGsTextureCmd : public CDListCmd {
      GS::CMemArea	&Texture;
   public:
      CSetGsTextureCmd(GS::CMemArea &tex) : Texture(tex) {}
      CDListCmd* Play() {
...
void
CTexManager::SetGsTexture( GS::CMemArea &area )
{
   GLContext.TextureChanged();
...
CMMTexture::CMMTexture( GS::tContext context )
   : CTexture( context ), pImageMem(NULL), XferImage(false), IsResident(false)
{
 ...
 */
void
CMMTexture::SetImage( tU128* imagePtr, tU32 w, tU32 h, GS::tPSM psm )
{
   if ( pImageMem ) {
 ...
 */
void
CMMTexture::SetImage( const GS::CMemArea &area )
{
...
void
CMMTexture::ChangePsm( GS::tPSM psm )
{
...
void
CMMTexture::Load( CSCDmaPacket& packet )
{
   mErrorIf( pImageMem == NULL,
	     "Trying to load a texture that hasn't been defined!" );
   if ( ! pImageMem->IsAllocated() ) {
...
void
CMMTexture::Load( CVifSCDmaPacket& packet )
{
   mErrorIf( pImageMem == NULL,
	     "Trying to load a texture that hasn't been defined!" );
   if ( ! pImageMem->IsAllocated() ) {
...
void
CMMTexture::Use( CSCDmaPacket& packet )
{
   if ( XferImage ) Load( packet );
   SendSettings( packet );
}
void
CMMTexture::Use( CVifSCDmaPacket& packet )
{
   if ( XferImage ) Load( packet );
   SendSettings( packet );
}
...
void
CMMTexture::BindToSlot( GS::CMemSlot &slot )
{
...
void
CMMClut::Load( CVifSCDmaPacket &packet )
{
 ...
 */
void
pglBindTextureToSlot( GLuint texId, pgl_slot_handle_t mem_slot )
{
   CTexManager &texManager = pGLContext->GetTexManager();
   CMMTexture &texture = texManager.GetNamedTexture(texId);
