...
void
CImmMaterial::SetSpecular( cpu_vec_xyzw specular )
{
...
void
CImmMaterial::LightsHaveSpecular()
{
   if ( Specular != cpu_vec_4(0,0,0,0) ) {
      GLContext.SpecularEnabledChanged();
...
void
CDListMaterial::SetAmbient( cpu_vec_xyzw ambient )
{
...
void
CDListMaterial::SetDiffuse( cpu_vec_xyzw diffuse )
{
   CDList &dlist = pGLContext->GetDListManager().GetOpenDList();
...
void
CDListMaterial::SetSpecular( cpu_vec_xyzw specular )
{
   CDList &dlist = pGLContext->GetDListManager().GetOpenDList();
...
void
CDListMaterial::SetEmission( cpu_vec_xyzw emission )
{
   CDList &dlist = pGLContext->GetDListManager().GetOpenDList();
...
void
CDListMaterial::SetShininess( float shine )
{
   CDList &dlist = pGLContext->GetDListManager().GetOpenDList();
...
void
CMaterialManager::Color( cpu_vec_xyzw color )
{
   CurColor = color;

   if ( UseColorMaterial ) {
      switch( ColorMaterialMode ) {
	 case GL_EMISSION:	CurMaterial->SetEmission( color ); break;
	 case GL_AMBIENT:	CurMaterial->SetAmbient( color ); break;
	 case GL_DIFFUSE:	CurMaterial->SetDiffuse( color ); break;
	 case GL_SPECULAR:	CurMaterial->SetSpecular( color ); break;
...
class CSetUseColorMaterialCmd : public CDListCmd {
      bool	UseCM;
   public:
      CSetUseColorMaterialCmd( bool use ) : UseCM(use) {}
      CDListCmd* Play() {
...
void
CMaterialManager::SetUseColorMaterial( bool yesNo )
{
   if ( ! InDListDef ) {
      UseColorMaterial = yesNo;
      if ( yesNo ) Color( CurColor );
      GLContext.CurMaterialChanged();
   }
   else {
...
class CSetColorMatModeCmd : public CDListCmd {
      GLenum	Mode;
   public:
      CSetColorMatModeCmd( GLenum mode ) : Mode(mode) {}
      CDListCmd* Play() {
...
void
CMaterialManager::SetColorMaterialMode( GLenum mode )
{
   if ( ! InDListDef ) {
