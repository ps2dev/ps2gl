...
void
CDListGeomManager::BeginDListDef()
{
...
void
CDListGeomManager::BeginGeom( GLenum mode )
{
   if ( Prim != mode ) PrimChanged(mode);
...
void
CDListGeomManager::Vertex( cpu_vec_xyzw newVert )
{
...
class CSetNormalCmd : public CDListCmd {
      cpu_vec_xyz	Normal;
   public:
      CSetNormalCmd( cpu_vec_xyz normal ) : Normal(normal) {}
      CDListCmd* Play() {
...
void
CDListGeomManager::Normal( cpu_vec_xyz normal )
{
...
class CColorCmd : public CDListCmd {
   ...
   public:
      CColorCmd( cpu_vec_xyzw color ) : CurColor(color) {}
      CDListCmd* Play() {
...
void
CDListGeomManager::Color( cpu_vec_xyzw color )
{
   if ( InsideBeginEnd ) {
      *CurColorBuf += color;
...
class CSetTexCoordCmd : public CDListCmd {
      float	U, V;
   public:
      CSetTexCoordCmd( float u, float v ) : U(u), V(v) {}
      CDListCmd* Play() {
...
void
CDListGeomManager::TexCoord( float u, float v )
{
   if ( InsideBeginEnd ) {
...
void
CDListGeomManager::DrawArrays( GLenum mode, int first, int count )
{
   if ( Prim != mode ) PrimChanged(mode);
...
void
CDListGeomManager::DrawIndexedArrays( GLenum primType,
...
class CUpdateRendererContextCmd : public CDListCmd {
      GLenum PrimType;
   public:
      CUpdateRendererContextCmd( GLenum type ) : PrimType(type) {}
      CDListCmd* Play() {
...
class CUpdatePrimCmd : public CDListCmd {
      unsigned char	Prim;
   public:
      CUpdatePrimCmd( unsigned char prim ) : Prim(prim) {}
      CDListCmd* Play() {
...
void
CDListGeomManager::PrimChanged( unsigned char prim )
{
...
class CUpdateRendererCmd : public CDListCmd {
   ...
   public:
	 ...
	 : PerVtxColors(pvColors), ArrayType(type) {}
      CDListCmd* Play() {
	 CImmGeomManager &gmanager = pGLContext->GetImmGeomManager();
...
void
CDListGeomManager::DrawingLinearArray()
{
...
void
CDListGeomManager::DrawingIndexedArray()
{
   if ( ! LastArrayAccessIsValid || ! LastArrayAccessWasIndexed ) {
      GLContext.ArrayAccessChanged();
      LastArrayAccessIsValid = true;
...
void
CDListGeomManager::CommitNewGeom()
{
...
class CDrawArraysCmd : public CDListCmd {
   ...
   public:
	 ...
	 : Geometry(block), IsCached(false), DList(dlist), RenderPacket(NULL)
      {}
...
class CDrawIndexedArraysCmd : public CDListCmd {
   ...
   public:
	 ...
	 if ( ! IsCached
	    ...
	    IsTexEnabled = texEnabled; IsLightingEnabled = lEnabled;
...
void
CDListGeomManager::DrawBlock( CGeometryBlock &block )
{
...
class CEnableCustomCmd : public CDListCmd {
      tU64	Flag;
   public:
      CEnableCustomCmd( tU64 flag ) : Flag(flag) {}
      CDListCmd* Play() {
...
void
CDListGeomManager::EnableCustom( tU64 flag )
{
...
class CDisableCustomCmd : public CDListCmd {
      tU64	Flag;
   public:
      CDisableCustomCmd( tU64 flag ) : Flag(flag) {}
      CDListCmd* Play() {
...
void
CDListGeomManager::DisableCustom( tU64 flag )
{
